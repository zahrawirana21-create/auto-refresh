...
// Railway/VPS kadang tidak menempelkan Variables ke container. Supaya tidak
// perlu bergantung pada dashboard, service ini juga membaca file `config.env`
// (atau `.env`) yang ikut di-upload bersama kode. Env asli tetap menang.
(function loadFileConfig() {
  ...
  for (const name of ["config.env", ".env"]) {
    ...
    for (const rawLine of fs.readFileSync(file, "utf8").split(/\r?\n/)) {
      ...
      if (process.env[key]) continue; // env dashboard tetap prioritas
      process.env[key] = val;
      loaded++;
    }
    console.log(`[refresher] konfigurasi dibaca dari ${name} (${loaded} nilai)`);
    break;
  }
})();
...
// ---- Work stealing: kalau shard lain gagal/mati, akun mendesaknya boleh
// diambil alih instance yang sehat setelah terbengkalai STEAL_AFTER_MS.
const STEAL_ENABLED = process.env.STEAL_ENABLED !== "0";
const STEAL_AFTER_MS = Math.max(30000, Number(process.env.STEAL_AFTER_MS || 120000));
const STEAL_MAX_PER_CYCLE = Math.max(1, Number(process.env.STEAL_MAX_PER_CYCLE || 5));
// Lease klaim di database supaya dua instance tidak menggarap akun yang sama.
const CLAIM_LEASE_MS = Math.max(60000, Number(process.env.CLAIM_LEASE_MS || 5 * 60 * 1000));
// Refresh preemptif: token diperbarui jauh sebelum kedaluwarsa supaya akun
// tidak pernah sampai berstatus mati.
const PREEMPT_MARGIN_MS = Math.max(5 * 60 * 1000, Number(process.env.PREEMPT_MARGIN_MS || 20 * 60 * 1000));
// Penjaga RAM: jangan pernah start Chromium kalau memori bebas sudah kritis.
const MIN_FREE_MEM_MB = Math.max(0, Number(process.env.MIN_FREE_MEM_MB || 260));
const MEM_WAIT_MS = Math.max(2000, Number(process.env.MEM_WAIT_MS || 6000));
const MEM_WAIT_TRIES = Math.max(0, Number(process.env.MEM_WAIT_TRIES || 5));
      ...
      try {
        return await chromium.launch({
          ...
          ...(proxy
            ...
            : {}),
          // Salinan baru setiap launch: Playwright memutasi array args yang
          // diberikan (menggabungkan nilai --disable-blink-features), sehingga
          // array bersama akan membengkak tiap siklus sampai command line rusak.
          args: [...LAUNCH_ARGS],
        });
      } catch (err) {
        lastErr = err;
...
function priority(r) {
  ...
  // Akun yang memang perlu refresh harus selalu berada di depan. Sebelumnya
  // nilainya terbalik sehingga akun aktif diproses berulang dan akun mati
  // kelaparan di belakang antrean.
  return broken ? 0 : 1;
}

function isBroken(r) {
  const st = String(r.status || "").toLowerCase();
  return r.is_active === false || ["needs_refresh", "expired", "error", "invalid"].includes(st);
}
...
function successCooldown(expIso) {
  ...
  // Sisakan PREEMPT_MARGIN_MS (default 20 menit) sebelum kedaluwarsa supaya akun
  // selalu di-refresh lebih awal dan tidak pernah sempat berstatus mati.
  const safe = exp - Date.now() - PREEMPT_MARGIN_MS;
  return Math.max(60000, Math.min(ACCOUNT_COOLDOWN_MS, safe));
}

/** Akun milik shard lain yang terbengkalai (worker-nya gagal/mati) boleh diambil alih. */
function stealable(r, now = Date.now()) {
  if (!STEAL_ENABLED || SHARD_TOTAL <= 1) return false;
  if (ownedByShard(r)) return false;
  if (!isUrgent(r, now)) return false;
  const lease = r.refresh_lease_until ? new Date(r.refresh_lease_until).getTime() : 0;
  if (lease > now) return false; // sedang dikerjakan worker lain
  const progress = Math.max(
    r.last_refresh_at ? new Date(r.last_refresh_at).getTime() : 0,
    r.updated_at ? new Date(r.updated_at).getTime() : 0,
  );
  return now - progress >= STEAL_AFTER_MS;
}

/** Klaim akun di database (lease) supaya tidak dobel dikerjakan antar instance. */
async function claimAccount(id) {
  try {
    const res = await sync("", {
      method: "POST",
      body: JSON.stringify({ action: "claim", id, worker: WORKER_NAME, lease_ms: CLAIM_LEASE_MS }),
    });
    return res?.claimed !== false;
  } catch (e) {
    log("⚠️  klaim akun gagal", id, e.message);
    return true; // jangan blokir refresh hanya karena lease gagal
  }
}

async function releaseAccount(id) {
  try {
    await sync("", { method: "POST", body: JSON.stringify({ action: "release", id }) });
  } catch { /* abaikan */ }
}

/** RAM bebas (MB) — dipakai untuk mencegah Chromium start saat memori kritis. */
function freeMemMb() {
  try {
    return Math.round(require("os").freemem() / (1024 * 1024));
  } catch {
    return 99999;
  }
}

/** Tunggu sampai RAM cukup; false berarti tetap kritis (akun ditunda ke batch berikutnya). */
async function waitForMemory() {
  if (!MIN_FREE_MEM_MB) return true;
  for (let i = 0; i <= MEM_WAIT_TRIES; i++) {
    const free = freeMemMb();
    if (free >= MIN_FREE_MEM_MB) return true;
    if (global.gc) { try { global.gc(); } catch {} }
    log(`🧠 RAM bebas ${free}MB < ${MIN_FREE_MEM_MB}MB — tunggu ${MEM_WAIT_MS}ms`);
    await sleep(MEM_WAIT_MS);
  }
  ...
  try {
    ...
    const candidates = requestedIds.length
      ...
      : (() => {
          ...
          const mine = all.filter(ownedByShard);
          // Ambil alih akun mendesak milik shard yang sedang gagal/mati.
          const stolen = all
            .filter((r) => stealable(r, nowTs))
            .sort(
              (a, b) =>
                (a.expires_at ? new Date(a.expires_at).getTime() : 0) -
                (b.expires_at ? new Date(b.expires_at).getTime() : 0),
            )
            .slice(0, STEAL_MAX_PER_CYCLE)
            .map((r) => ({ ...r, _stolen: true }));
          if (stolen.length) log(`🤝 ambil alih ${stolen.length} akun mendesak dari shard lain`);
          return [...mine, ...stolen];
        })();
    ...
    // Hanya akun berstatus rusak yang boleh menembus cooldown sukses. Akun
    // aktif dengan token baru yang umurnya < URGENT_WINDOW_MS harus tetap
    // menghormati cooldown agar tidak diproses ulang tiap 45 detik.
    const eligible = requestedIds.length
      ? sorted
      : sorted.filter((r) => {
          const until = cooldown.get(r.id) || 0;
          if (isBroken(r)) return urgentReady(r, now);
          return until <= now;
        });
    ...
    const worker = async () => {
      while (pending.length) {
        ...
        // Penjaga RAM: kalau memori kritis, akun ini ditunda ke batch berikutnya
        // daripada memaksa Chromium start lalu crash.
        if (!(await waitForMemory())) {
          automaticBacklog += pending.length + 1;
          pending.length = 0;
          log("🧠 RAM masih kritis — batch dihentikan, dilanjutkan setelah memori lega");
          return;
        }
        // Klaim lease supaya instance lain tidak mengerjakan akun yang sama.
        if (!(await claimAccount(account.id))) {
          log("↪️  akun sedang dikerjakan worker lain, dilewati:", label);
          continue;
        }
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  if (!SYNC_URL || !REFRESHER_SECRET) {
    ...
    log(
      `   diagnosa: SYNC_URL=${SYNC_URL ? "ADA" : "KOSONG"}, ` +
        `REFRESHER_SECRET=${REFRESHER_SECRET ? "ADA" : "KOSONG"}, ` +
        `SUPABASE_ANON_KEY=${ANON_KEY ? "ADA" : "KOSONG"}`,
    );
    log("   perbaikan: isi file config.env di folder ini, lalu upload & redeploy.");
    return;
  }
  log(`✅ konfigurasi lengkap — siklus otomatis aktif (shard ${SHARD_INDEX + 1}/${SHARD_TOTAL}, worker ${WORKER_NAME})`);
